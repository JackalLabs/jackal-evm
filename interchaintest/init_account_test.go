...
func (s *ContractTestSuite) TestInitAccount() {
	...
	s.Run(fmt.Sprintf("TestCreateBindingsSuccess-%s", encoding), func() {
		...
		factoryExecuteMsg := factorytypes.ExecuteMsg{
			InitAccount: &factorytypes.ExecuteMsg_InitAccount{
				EvmAddress: &aliceEvmAddress,
			},
		}

		res1, _ := s.ChainB.ExecuteContract(ctx, s.UserB.KeyName(), factoryContractAddress, factoryExecuteMsg.ToString(), "--gas", "500000", "--amount", "200000000ujkl")
		...
		bindingsAddress, addressErr := testsuite.GetBindingsAddressFromFactoryMap(ctx, s.ChainB, factoryContractAddress, aliceEvmAddress)
		s.Require().NoError(addressErr)
		fmt.Println(bindingsAddress)
		...
		// Get entire map to make sure there's still only 1 bindings address
		...
		if len(decodedBindingsMap) != 1 {
			log.Fatalf("Unexpected number of bindings found: expected 1, got %d", len(decodedBindingsMap))
		}

		// Extract the only binding entry
		binding := decodedBindingsMap[0]
		if len(binding) != 2 {
			log.Fatalf("Invalid binding format: %v", binding)
		}

		// Log the only value in the map
		userAddress := binding[0]
		userBindingsAddress := binding[1]
		logger.LogInfo("User Address:", userAddress)
		logger.LogInfo("User Bindings Address:", userBindingsAddress)
